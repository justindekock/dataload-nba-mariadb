...
# class TeamData:
#     def __init__(self, raw_df, p=False):
#         self.p=p
#         self.table_dfs = []
#         self.raw_df = raw_df
#         self.clean_df = clean_raw_df(self.raw_df)
...
#             if self.p:
#                 df_g = df.copy()
#                 df = df_g.groupby(['game_id', 'team']).sum('pts').reset_index(drop=True)
#                 print(df)
...
# # team, season, game, t_game, t_box, t_shtg
# # 
    
# # PlayerData class produces datasets for the following tables: 
# # player, p_game, p_box, p_shtg
# # TODO - LOW PRIORITY - CONVERT THIS TO INHERIT FROM TEAM
# class PlayerData(TeamData):
#     def __init__(self, raw_df, p=True):
#         super().__init__(raw_df, p)
...
#             self.pgame_df = self.get_pgame_df(self.tgame_df) # now inherits TeamData parent instead of having to pass
#             self.print_dfs()
    
#     def get_player_df(self):
#         clean_df = self.clean_df[['player_id', 'player_name']].drop_duplicates()
#         self.table_dfs.append(clean_df)
#         return clean_df
    
#     def get_tgame_df(self):
        
#         tgame_df_rec = self.clean_df[['game_id', 'season_id', 'team_id', 'game_date', 'matchup',
#                    'pts', 'pm', 'wl', 'mins']].copy() 
        
#         tgame_t = self.clean_df[['game_id', 'season_id', 'team_id', 'player_id', 'game_date', 'matchup',
#                    'pts', 'pm', 'wl', 'mins']].copy()
        
#         tgame_df = tgame_t.groupby(['game_id', 'season_id', 'team_id', 
#                                     'game_date', 'matchup']).sum(['pts', 'pm', 'mins']).reset_index(drop=True)
#         print(tgame_df)
...
#     def get_pgame_df(self):
#         pl_df = self.clean_df[['game_id', 'season_id', 'team_id', 'player_id', 'mins', 'pts']].copy()
#         joined = pl_df.merge(self.team_df, on=['game_id', 'season_id', 'team_id'])
...
#            'fg3a', 'ftm', 'fta', 'fg_pct', 'fg3_pct', 'ft_pct']].copy() 
#         self.table_dfs.append(pshtg_df)
#         return pshtg_df
